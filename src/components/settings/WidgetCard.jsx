...
export default function WidgetCard({ platform, url, options, cardTheme, preview = false }) {
  ...
  const containerStyle = options?.useCardStyle
    ? {
        backgroundColor: hexToRgba(cardTheme?.cardColor, cardTheme?.cardOpacity),
  ...
  const stacked = platform.id === 'github' && extraOn;
  ...
  const linkProps = preview ? {} : { href: url, target: '_blank', rel: 'noreferrer' };
    ...
    >
      {showBanner && (
        <div
          ...
          style={{ background: `linear-gradient(to bottom, ${hexToRgba(platform.brandColor, 35)}, transparent)` }}
        />
      )}
      <div
